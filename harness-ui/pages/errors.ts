...
import { fetchErrorSummary, fetchErrors } from "../lib/api.ts";
...
export async function renderErrors(
  query?: { severity?: string; component?: string },
  projectId?: string,
): Promise<string> {
  ...
  const rows = errorList
    .map((e: any) => {
      const icon = SEVERITY_ICONS[e.severity] ?? "•";
      const color = SEVERITY_COLORS[e.severity] ?? "#6b7280";
      const sessionLink = e.session_id
        ? `<a class="err-session-link" href="/sessions/${encodeURIComponent(e.session_id)}">${escapeHtml(e.session_id.split("/").pop())}</a>`
        : "";
  ...
  return layout(content, {
    title: "Errors",
    activePath: projectId ? `/projects/${projectId}/errors` : "/errors",
    projectId,
