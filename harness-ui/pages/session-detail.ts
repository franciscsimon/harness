  ...
  const rows = events
    .map((ev: any) => {
      ...
      const fieldHtml = Object.entries(fields)
        .map(
          ([k, v]) => `<span class="field-pair"><span class="field-k">${escapeHtml(k)}:</span> ${escapeHtml(v)}</span>`,
        )
        .join(" ");

      const ctxBytes =
        ev.provider_payload_bytes != null && ev.provider_payload_bytes !== ""
          ? Number(ev.provider_payload_bytes)
          : null;
      let ctxAttrs = "";
      let ctxClasses = "";
      let ctxBadgeHtml = "";
