...
export async function renderDashboard(projectId?: string): Promise<string> {
  ...
  if (!data) {
    return layout('<p class="empty-msg">Dashboard unavailable — cannot reach event logger API.</p>', {
      title: "Dashboard",
      activePath: projectId ? `/projects/${projectId}/dashboard` : "/dashboard",
      projectId,
  ...
  const overallErrorRate = data.overallErrorRate != null ? (data.overallErrorRate * 100).toFixed(1) : "0";
  const avgDuration =
    sessions.length > 0
