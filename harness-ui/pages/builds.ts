...
export async function renderBuilds(projectId?: string): Promise<string> {
  ...
  } catch { /* intentionally silent — page renders empty state */ }
...
export async function renderBuildDetail(buildId: string, projectId?: string): Promise<string> {
  let build: any = null;
  try {
    const r = await fetch(`${BUILD_API}/api/builds/${encodeURIComponent(buildId)}`, { signal: AbortSignal.timeout(10000) });
    if (r.ok) build = await r.json();
  } catch { /* intentionally silent — page renders not-found */ }

  if (!build) {
    return layout(`<main class="container"><h1>Build Not Found</h1><p>No build with ID <code>${escapeHtml(buildId)}</code></p></main>`, {
      title: "Build Not Found", projectId, activeSection: "builds",
    });
  }
