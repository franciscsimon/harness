...
const _$model = document.getElementById("model-select");
...
function connect() {
  ...
  const wsHost = window.CHAT_WS_HOST || location.host;
  ...
  ws.onopen = () => {
    ...
    if (isNew) {
      ...
      if (urlSession) {
        sessionStorage.setItem("pi-chat-sessionFile", urlSession);
        history.replaceState(null, "", "/chat");
      }
...
function handleMessage(msg) {
  switch (msg.type) {
    ...
    case "cwd":
      ...
      break;
    case "session_list":
      break; // future: session picker
    case "ui:notify":
      showNotify(msg.message, msg.level);
      break;
    case "ui:status":
      showExtStatus(msg.key, msg.text);
      break;
    case "ui:confirm":
      handleConfirm(msg);
      break;
    ...
    case "ui:input":
      ...
      break;
    case "compact_done":
      showSystemMsg(`📦 Compacted: ${msg.summary}`);
      break;
    case "context_usage":
    ...
    case "auto_compact_start":
      showSystemMsg(`⚙️ Auto-compacting (${msg.reason})…`);
      break;
    ...
    case "auto_retry_start":
      showSystemMsg(
        `🔄 Retrying (${msg.attempt}/${msg.maxAttempts}) in ${(msg.delayMs / 1000).toFixed(0)}s — ${msg.error}`,
      );
      break;
    case "auto_retry_end":
    ...
    // P4: Branching
    case "fork_points":
      break; // future: fork picker UI
    case "command_list":
      showCommandList(msg.commands);
      break;
    case "forked":
      showSystemMsg(`🌿 Forked → ${msg.sessionFile}`);
      break;
    // P6: Export/copy
    case "exported_html":
      showSystemMsg(`📄 Exported: ${msg.path}`);
      break;
    case "copied_text":
      if (msg.text) {
        navigator.clipboard?.writeText(msg.text);
        showSystemMsg("📋 Copied to clipboard");
...
function updateSessionInfo(msg) {
  ...
  $sbSessionName.innerHTML = msg.sessionName ? `<span class="sb-row-label">Name:</span> ${esc(msg.sessionName)}` : "";
  if (msg.sessionFile) {
...
function renderDecisions(decisions) {
  ...
  if (!decisions.length) {
    container.innerHTML = '<div class="sb-empty">No decisions yet</div>';
    return;
  }
...
function renderArtifacts(artifacts) {
  ...
  if (!artifacts.length) {
    container.innerHTML = '<div class="sb-empty">No artifacts yet</div>';
    return;
  }
...
function toolSummaryHint(name, input) {
  ...
  switch (name) {
    case "Read":
    case "read":
      return input.path ? shortenPath(input.path) : "";
    case "Write":
    case "write":
      return input.path ? shortenPath(input.path) : "";
    case "Edit":
    ...
    case "delegate":
      return input.agent ? `→ ${input.agent}` : input.task ? input.task.slice(0, 60) : "";
    case "log_decision":
      return input.task ? input.task.slice(0, 60) : "";
    case "plan_chunks":
      return input.action ?? "";
    case "quality_check":
      return input.path ? shortenPath(input.path) : "";
...
function endTool(msg) {
  ...
  if (statusEl) {
    ...
    statusEl.className = msg.isError ? "chat-tool-status tool-error" : "chat-tool-status tool-ok";
  }
...
function showExtStatus(key, text) {
  ...
  if (!text) {
    bar.querySelectorAll(`[data-key="${key}"]`).forEach((e) => e.remove());
    return;
  }
  let span = bar.querySelector(`[data-key="${key}"]`);
  if (!span) {
    span = el("span", "chat-ext-status-item");
...
$newBtn.addEventListener("click", () => {
  window.open(`${location.origin}/chat?new=1`, "_blank");
});
...
// Sidebar buttons
document
  .getElementById("wf-advance")
  .addEventListener("click", () => wsSend({ type: "prompt", text: "/workflow advance" }));
document.getElementById("wf-skip").addEventListener("click", () => wsSend({ type: "prompt", text: "/workflow skip" }));
document
  .getElementById("wf-abandon")
  .addEventListener("click", () => wsSend({ type: "prompt", text: "/workflow abandon" }));
...
document.getElementById("btn-reload").addEventListener("click", () => {
  wsSend({ type: "reload" });
  showSystemMsg("🔄 Reloading extensions…");
});
document.getElementById("btn-compact").addEventListener("click", () => {
  wsSend({ type: "compact" });
  showSystemMsg("📦 Compacting…");
});

// Sidebar toggles
$chkAutoCompact.addEventListener("change", () =>
  wsSend({ type: "set_auto_compact", enabled: $chkAutoCompact.checked }),
);
...
function sendPrompt() {
  ...
  if (text === "/reload") {
    wsSend({ type: "reload" });
    showSystemMsg("🔄 Reloading…");
    $input.value = "";
    autoResize();
    return;
  }
  if (text === "/stats") {
    wsSend({ type: "get_stats" });
    $input.value = "";
    autoResize();
    return;
  }
  if (text === "/help" || text === "/commands") {
    showCommandListInChat = true;
  ...
  for (const raw of lines) {
    if (raw.startsWith("```")) {
      if (inCode) {
        ...
        out.push(`<pre class="chat-code"><code${codeLang ? ` class="lang-${esc(codeLang)}"` : ""}>`);
        inCode = true;
      }
      continue;
    }
...
function showCommandList(commands) {
  ...
  if (acPendingFilter !== null) {
    const f = acPendingFilter;
    acPendingFilter = null;
    showAutocomplete(f);
  }
...
function showAutocomplete(filter) {
  ...
  const matches = filter ? knownCommands.filter((c) => c.name.startsWith(filter)) : knownCommands;
...
function showDialogOverlay({ title, body, buttons, options, inputPlaceholder, onResult, fallback }) {
  ...
  overlay.addEventListener("keydown", (e) => {
    if (e.key === "Escape") respond(fallback);
  });
  overlay.tabIndex = -1;
