  ...
  pi.registerCommand("parallel", {
    ...
    handler: async (args, ctx) => {
      ...
      if (cmd === "start") {
        ...
        if (!task) {
          ctx.ui.notify("Usage: /parallel start <count> <task description>", "error");
          return;
        }
      ...
      if (cmd === "pick") {
        ...
        if (!attempt) {
          ctx.ui.notify(`Attempt #${id} not found.`, "error");
          return;
        }
      ...
      if (cmd === "compare") {
        if (attempts.length === 0) {
          ctx.ui.notify("No attempts. Use /parallel start.", "info");
          return;
        }
        pi.sendUserMessage(
      ...
      // Default: status
      if (attempts.length === 0) {
        ctx.ui.notify("No parallel attempts. Use /parallel start <n> <task>.", "info");
        return;
      }
