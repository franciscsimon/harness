...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("dump", {
    ...
    handler: async (args, ctx) => {
      ...
      if (cmd === "save") {
        if (dumps.length === 0) {
          ctx.ui.notify("No dumps to save.", "info");
          return;
        }
      ...
      if (cmd === "list") {
        if (dumps.length === 0) {
          ctx.ui.notify("No dumps yet. Use /dump <your thoughts>", "info");
          return;
        }
      ...
      if (cmd === "ask") {
        if (dumps.length === 0) {
          ctx.ui.notify("Dump something first: /dump <thoughts>", "info");
          return;
        }
        const allText = dumps.map((d) => d.text).join("\n\n");
        pi.sendUserMessage(`Here's my mind dump. Ask me clarifying questions to surface gaps:\n\n${allText}`, {
          deliverAs: "followUp",
        });
        return;
      }
      ...
      if (!text) {
        ctx.ui.notify("Usage: /dump <your unstructured thoughts> | save | list | ask", "error");
        return;
      }
      dumps.push({ text, ts: Date.now() });
      ctx.ui.notify(
        `🧠 Dump #${dumps.length} captured (${text.length} chars). Use /dump ask to have AI clarify.`,
        "success",
      );
    },
  });
