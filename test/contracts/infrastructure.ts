...
async function main() {
  ...
  primaryAlive?.includes("Alive") ? pass("Primary is alive") : fail("Primary is alive", `got: ${primaryAlive}`);
  ...
  replicaAlive?.includes("Alive") ? pass("Replica is alive") : fail("Replica is alive", `got: ${replicaAlive}`);
  ...
  s3Status !== null ? pass("Garage S3 API responding") : fail("Garage S3 API responding", "connection refused");
