...
import {
  ...
  detectRetryStorm,
} from "../canary-monitor/metrics.ts";
...
import {
  ...
  formatNumber,
  truncate,
  healthColor as uiHealthColor,
  relativeTime as uiRelativeTime,
} from "../harness-ui/lib/format.ts";
import {
  computeHealthScore as uiComputeHealthScore,
  healthColor as uiHealthColor2,
  healthLabel as uiHealthLabel,
} from "../harness-ui/lib/health.ts";
import { b, n, t } from "../lib/db.ts";
import { captureError, unflushedErrorCount } from "../lib/errors.ts";
import { personAgent, piId, piRef, softwareAgent, xsdBool, xsdLong } from "../lib/jsonld/context.ts";
import { projectId } from "../project-registry/identity.ts";
import { normalizeGitUrl } from "../project-registry/normalize.ts";
import { parseClientMessage, send } from "../web-chat/lib/ws-protocol.ts";
import { ALL_EVENT_NAMES, routeEvent } from "../xtdb-event-logger/router.ts";
import { flushSampler, shouldCapture } from "../xtdb-event-logger/sampling.ts";
import { safeJsonSize, trunc, uuid } from "../xtdb-event-logger/util.ts";
import { computeLineDiff, renderDiffHtml } from "../xtdb-event-logger-ui/lib/diff.ts";
...
import {
  ...
  getPopulatedFields,
  relativeTime,
} from "../xtdb-event-logger-ui/lib/format.ts";
...
import { accumulate, createRunState } from "../xtdb-projector/accumulator.ts";
import { inputSummary, isMutation } from "../xtdb-projector/mutations.ts";
import { projectChanges, projectReasoning, projectResult, projectTask } from "../xtdb-projector/projectors.ts";
...
assert(
  "just now",
  relativeTime(String(now - 5000)).includes("s ago") || relativeTime(String(now - 5000)).includes("just now"),
);
assert(
  "minutes ago",
  relativeTime(String(now - 120_000)).includes("m ago") || relativeTime(String(now - 120_000)).includes("min"),
);
assert(
  "hours ago",
  relativeTime(String(now - 7_200_000)).includes("h ago") || relativeTime(String(now - 7_200_000)).includes("hour"),
);
...
eq(
  "https with port",
  normalizeGitUrl("https://gitlab.example.com:8443/group/project.git"),
...
assert(
  "uiRelativeTime recent",
  uiRelativeTime(nowTs - 5000).includes("s ago") || uiRelativeTime(nowTs - 5000).includes("just now"),
);
...
const uiWorst = uiComputeHealthScore({
  errorRate: 1,
  turnCount: 50,
...
const withRender = renderTable([{ key: "x", label: "X", render: (v: any) => `<b>${v}</b>` }], [{ x: "bold" }]);
...
assert(
  "getDisplayFields tool_call has tool_name",
  Object.keys(displayFields).some(
    (k) => k.includes("tool") || k.includes("name") || displayFields[k]?.includes("read"),
  ),
);
...
const popEmpty = getPopulatedFields({
  _id: "ev:x",
  event_name: "x",
  category: "a",
  can_intercept: false,
...
assert(
  "tfr empty has value/threshold/alert/message",
  "value" in tfr0 && "threshold" in tfr0 && "alert" in tfr0 && "message" in tfr0,
);
...
const tfrHigh = computeToolFailureRate(Array(10).fill({ is_error: true }), thresholds);
...
const rsStorm = detectRetryStorm(Array(5).fill({ tool_name: "read" }), thresholds);
...
const rsMixed = detectRetryStorm([{ tool_name: "read" }, { tool_name: "write" }, { tool_name: "read" }], thresholds);
...
assert(
  "commit habit has fields",
  "name" in ch0 && "alert" in ch0 && "value" in ch0 && "threshold" in ch0 && "prompt" in ch0,
);
eq("commit habit no edits not alerting", ch0.alert, false);

const chMany = checkCommitHabit(Array(20).fill("write"), [], hThresholds);
eq("commit habit many edits alerts", chMany.alert, true);
assert("commit habit has prompt", chMany.prompt.length > 0);

const chAfterCommit = checkCommitHabit(["write", "write"], ["git commit -m 'done'"], hThresholds);
...
const thMany = checkTestHabit(Array(20).fill("edit"), [], hThresholds);
...
const scDupes = checkScopeCreep(Array(20).fill("same.ts"), hThresholds);
